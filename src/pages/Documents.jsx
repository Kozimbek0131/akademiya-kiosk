...
const Documents = () => {
  ...
  // Kategoriyalar — t() har render da qayta ishlaydi
  const categories = [
  ...
  const getTitle = (titleObj) => titleObj[language] || titleObj.uz;
        ...
        <div className="grid grid-cols-3 gap-4 bg-slate-800/50 p-2 rounded-3xl border border-white/10 shadow-inner">
          {categories.map(cat => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
              className={`flex flex-col items-center justify-center gap-2 py-4 px-2 rounded-2xl font-bold transition-all cursor-pointer ${activeCategory === cat.id ? 'bg-amber-500 text-slate-900 shadow-[0_5px_15px_rgba(245,158,11,0.4)] scale-[1.02]' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
            ...
            <div key={doc.id} className="bg-slate-800/60 backdrop-blur-md border border-white/10 p-5 md:p-6 rounded-[2rem] flex items-center justify-between gap-4 hover:border-amber-500/50 transition-all group">
              <div className="flex items-center gap-5 md:gap-6 flex-1 min-w-0">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <h3 className="text-lg md:text-xl font-bold text-white mb-1 truncate group-hover:text-amber-400 transition-colors">{getTitle(doc.title)}</h3>
              ...
              <button onClick={() => setSelectedDoc(doc)} className="bg-white/10 hover:bg-amber-500 hover:text-slate-900 text-white px-6 py-4 rounded-2xl font-black uppercase tracking-widest transition-all active:scale-95 shrink-0 border border-white/20 cursor-pointer">
                {t('btn_read')}
              </button>
            </div>
          )) : (
          ...
          <div className="flex items-center justify-between p-4 md:p-6 bg-slate-900 border-b border-white/10 shadow-2xl z-20">
            <div className="flex items-center gap-4 flex-1 min-w-0 pr-4">
              ...
              <div className="flex flex-col min-w-0">
                <h2 className="text-lg md:text-2xl font-bold text-white truncate">{getTitle(selectedDoc.title)}</h2>
                <span className="text-xs text-gray-400">{t('doc_viewing')}</span>
              </div>
            </div>
            <button onClick={() => setSelectedDoc(null)} className="flex items-center gap-2 bg-red-600 hover:bg-red-500 text-white px-6 py-3 md:px-8 md:py-4 rounded-2xl font-black uppercase tracking-widest transition-all active:scale-95 shrink-0 cursor-pointer">
