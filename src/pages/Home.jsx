...
const Transport = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-900 relative overflow-x-hidden select-none text-white">
      ...
      {/* HEADER - z-index 50 qidiruv ro'yxati uchun muhim */}
      ...
      <div className="relative z-10 flex-1 p-4 md:p-6 flex flex-col md:flex-row gap-6 overflow-y-auto md:overflow-hidden">
        ...
        {/* CHAP TOMON - QIDIRUV VA TUGMALAR */}
        <div className="w-full md:w-[380px] lg:w-[420px] flex flex-col gap-4 md:gap-6 shrink-0 relative z-40">
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 md:p-6 rounded-2xl md:rounded-3xl shadow-xl relative">
            <label className="text-white text-sm md:text-lg font-bold mb-2 md:mb-3 block pl-1">Qayerga borasiz?</label>
            <form onSubmit={handleSearch} className="relative">
              <input 
                ...
                className="w-full bg-slate-900/90 text-white border-2 border-blue-500/50 rounded-xl md:rounded-2xl py-3 md:py-4 pl-10 md:pl-12 pr-10 md:pr-12 text-base md:text-xl focus:outline-none focus:border-blue-400"
              />
              <FaSearch className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-gray-400 text-base md:text-xl" />
              {destination && (
                <button type="button" onClick={clearSearch} className="absolute right-3 md:right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white p-2">
                  <FaTimes />
                </button>
              )}
            </form>

            {/* MANZILLAR RO'YXATI - z-index 100 */}
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-slate-800 border border-white/20 rounded-xl md:rounded-2xl shadow-2xl z-[100] max-h-48 md:max-h-60 overflow-y-auto">
                {suggestions.map((item, index) => (
                  ...
                  >
                    ...
                    <span className="truncate text-xs md:text-base font-medium block w-full">{item.display_name.split(',')[0]}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="bg-slate-800/50 border border-white/10 p-4 md:p-6 rounded-2xl md:rounded-3xl flex-1 flex flex-col min-h-[300px]">
            <h3 className="text-gray-400 font-bold uppercase tracking-widest mb-3 md:mb-4 text-[10px] md:text-sm pl-1">Tezkor yo'nalishlar</h3>
            <div className="flex flex-col gap-2 md:gap-3 overflow-y-auto custom-scrollbar pr-1">
              {[
                ...
                <button key={idx} onClick={() => updateMapRoute(item.target)} className="flex items-center gap-3 md:gap-4 bg-white/5 p-3 md:p-4 rounded-xl md:rounded-2xl hover:bg-blue-600 hover:text-white transition-all border border-white/5 group text-left shrink-0">
        ...
        <div className="flex-1 bg-white rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border-2 md:border-4 border-slate-700 relative z-0 min-h-[450px] md:h-full mb-6 md:mb-0">
           <iframe 
