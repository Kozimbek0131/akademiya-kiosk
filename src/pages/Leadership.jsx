    ...
    <div className="h-screen flex flex-col bg-slate-900 relative overflow-hidden select-none text-white font-sans">
      ...
      <div className="relative z-10 flex-1 p-4 md:p-10 flex flex-col items-center overflow-y-auto custom-scrollbar touch-pan-y w-full">
        ...
        {leaders.filter(l => l.isChief).map(chief => (
          <div key={chief.id} className="w-full max-w-6xl bg-gradient-to-r from-slate-800 to-slate-900 border-2 border-yellow-500/50 rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 flex flex-col md:flex-row items-center gap-6 md:gap-12 shadow-[0_0_60px_rgba(234,179,8,0.15)] mb-8 md:mb-12 relative overflow-hidden shrink-0">
            ...
            <div className="flex-1 text-center md:text-left">
               ...
               <p className="text-lg md:text-3xl text-blue-400 font-bold mb-6 md:mb-8 border-b border-white/10 pb-4 md:pb-6 uppercase tracking-wide">
                 {chief.position}
               </p>
               
               {/* MUHIM O'ZGARISH: Telefon va pochta bloki (flex-wrap va whitespace-nowrap qo'shildi) */}
               <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 md:gap-6">
                 ...
                 <div className="flex items-center gap-3 bg-white/10 border border-white/20 px-5 py-3 md:px-6 md:py-4 rounded-2xl text-gray-200 shadow-inner">
                    <FaPhoneAlt className="text-green-400 text-xl md:text-3xl shrink-0" />
                    <span className="font-mono text-base md:text-2xl font-bold whitespace-nowrap">{chief.phone}</span>
                 </div>
                 
                 <div className="flex items-center gap-3 bg-white/10 border border-white/20 px-5 py-3 md:px-6 md:py-4 rounded-2xl text-gray-200 shadow-inner">
                    <FaEnvelope className="text-blue-400 text-xl md:text-3xl shrink-0" />
                    <span className="font-mono text-base md:text-2xl font-bold whitespace-nowrap">{chief.email}</span>
                 </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 w-full max-w-6xl pb-20">
          {leaders.filter(l => !l.isChief).map(deputy => (
            <div key={deputy.id} className="bg-slate-800/40 backdrop-blur-xl border border-white/10 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 flex flex-col items-center text-center hover:bg-slate-800/60 transition-all hover:border-blue-500/50 shadow-xl group">
              ...
              <p className="text-gray-400 text-sm md:text-xl mb-6 h-auto md:h-14 font-medium leading-tight">
                {deputy.position}
              </p>

              {/* O'rinbosarlar telefoni uchun ham whitespace-nowrap */}
              <div className="w-full bg-black/30 rounded-2xl p-4 border border-white/5">
                 <div className="flex items-center justify-center gap-3 text-green-400 text-lg md:text-2xl font-black font-mono whitespace-nowrap">
                    <FaPhoneAlt className="text-base md:text-xl shrink-0" /> {deputy.phone}
                 </div>
              </div>
            </div>
          ))}
