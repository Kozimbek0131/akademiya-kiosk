...
const Leadership = () => {
  ...
  return (
    ...
    <div className="h-screen flex flex-col bg-slate-900 relative overflow-hidden select-none text-white font-sans">
      ...
      {/* flex-1 va overflow-y-auto shu yerga berildi */}
      <div className="relative z-10 flex-1 p-4 md:p-10 flex flex-col items-center overflow-y-auto custom-scrollbar w-full">
        
        {/* 1. AKADEMIYA BOSHLIG'I */}
        {leaders.filter(l => l.isChief).map(chief => (
          <div key={chief.id} className="w-full max-w-6xl bg-gradient-to-r from-slate-800 to-slate-900 border-2 border-yellow-500/50 rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 flex flex-col md:flex-row items-center gap-6 md:gap-12 shadow-[0_0_60px_rgba(234,179,8,0.15)] mb-8 md:mb-12 relative overflow-hidden group shrink-0">
